...
exports.updateSubSection = async (req, res) => {
  try {
    ...
    // Fix: If sectionId is provided, return the updated section
    // If not, we need to find which section contains this subsection
    ...
    if (sectionId) {
      ...
      updatedSection = await Section.findOne({
    ...
    return res.json({
      success: true,
      data: updatedSection,
      message: "SubSection updated successfully", // Fixed message
    });
  } catch (error) {
    console.error("Error while updating the subsection"); // Fixed log message
    console.error(error);
    return res.status(500).json({
      success: false,
      error: error.message,
      message: "Error while updating the subsection", // Fixed message
    });
  }
};
